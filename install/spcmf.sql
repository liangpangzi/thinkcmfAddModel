...
CREATE TABLE `sp_attribute` (
  ...
  `validate_rule` varchar(255) NOT NULL COMMENT '验证规则',
  `validate_time` tinyint(1) unsigned NOT NULL COMMENT '验证时间',
  `error_info` varchar(100) NOT NULL COMMENT '出错提示',
  `validate_type` varchar(25) NOT NULL COMMENT '验证方式',
  `auto_rule` varchar(100) NOT NULL COMMENT '自动完成规则',
  `auto_time` tinyint(1) unsigned NOT NULL COMMENT '自动完成时间',
  `auto_type` varchar(25) NOT NULL COMMENT '自动完成方式',
  PRIMARY KEY (`id`),
  KEY `model_id` (`model_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='模型属性表' AUTO_INCREMENT=1 ;
...
INSERT INTO `sp_menu` VALUES(158, 54, 'Admin', 'Slide', 'ban', '', 1, 0, '禁用幻灯片', '', '', 0);
INSERT INTO `sp_menu` VALUES(159, 54, 'Admin', 'Slide', 'cancelban', '', 1, 0, '启用幻灯片', '', '', 0);
INSERT INTO `sp_menu` VALUES(160, 149, 'Admin', 'User', 'ban', '', 1, 0, '禁用管理员', '', '', 0);
INSERT INTO `sp_menu` VALUES(161, 149, 'Admin', 'User', 'cancelban', '', 1, 0, '启用管理员', '', '', 0);
INSERT INTO `sp_menu` VALUES(162, 0, 'Admin', 'Model', 'default', '', 1, 1, '模型管理', '', '', 0);
INSERT INTO `sp_menu` VALUES(163, 162, 'Admin', 'Model', 'index', '', 1, 1, '模型列表', '', '', 0);
-- --------------------------------------------------------
